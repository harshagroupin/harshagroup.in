...
export default function PropertiesManager() {
  ...
  const load = async () => {
    ...
    // Show all properties since display location is configurable via switches
    setProperties(data || []);
    setLoading(false);
  };
  ...
  const openEdit = (p: Property) => {
    ...
    const currentLocs = p.features && p.features.length > 0 ? p.features : [p.display_location || "homepage"];
    setEditItem({
      ...p,
      features: p.features || [],
  ...
  const openNew = () => {
    setEditItem({ ...emptyProperty, sort_order: properties.length + 1, display_location: "homepage" });
    setNewFeature("");
    setDialogOpen(true);
  };
          ...
          <div className="space-y-5 mt-4">
            ...
            <div className="space-y-2">
              <label className="text-sm text-muted-foreground flex items-center gap-1.5">
                <ImageIcon size={14} className="text-primary" /> Property Image
              </label>
              <ImageUploader
                bucket="hero-images"
            ...
            <div className="space-y-2">
              <label className="text-sm text-muted-foreground flex items-center gap-1.5">
                <Video size={14} className="text-blue-400" /> Property Video
              </label>
              <VideoUploader
                bucket="hero-images"
